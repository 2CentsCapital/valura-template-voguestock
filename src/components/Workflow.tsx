...
export default function Workflow() {
  ...
  const mediaInView = useInView(mediaRef, '-15% 0px');

  return (
    <section className="border-t border-gray-100 bg-white pt-16 pb-0 sm:pt-24" id="why">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 items-center gap-10 lg:grid-cols-12 lg:gap-12">
          ...
          >
            <img
              ...
              loading="lazy"
              decoding="async"
              className={`h-full w-full rounded-2xl object-cover transition-transform duration-[1600ms] ease-out ${
                mediaInView ? 'scale-100' : 'scale-110'
              }`}
            />
            <figcaption className="absolute right-4 bottom-4 left-4 flex items-center gap-3 rounded-2xl border border-gray-100 bg-white p-4 font-sans shadow-xl sm:right-6 sm:bottom-6 sm:left-6 sm:gap-4 sm:p-5">
          ...
          <ul className="flex flex-col gap-6 font-sans lg:col-span-6">
            {WHY.map(({ icon: Icon, title, text, tint }) => (
              <li
                key={title}
                className="flex flex-col items-start gap-5 rounded-3xl border border-gray-100 bg-brand-light p-6 transition duration-300 hover:shadow-lg sm:flex-row sm:gap-6"
              >
        ...
        <div aria-hidden="true" className="mt-20 -mx-4 w-[calc(100%+2rem)] sm:mx-0 sm:w-full">
