...
const logoList = Object.values(logoModules).map(mod => mod.default);

const DEFAULT_IMAGES: ImageObj[] = logoList.map((src, i) => ({
  src,
  alt: `Logo ${i + 1}`
}));
...
interface DomeItem extends Coord {
  src: string;
  alt: string;
}

function buildItems(pool: (string | ImageObj)[], seg: number): DomeItem[] {
...
export default function DomeGallery({
  ...
  const focusedElRef = useRef<HTMLElement>(null);
