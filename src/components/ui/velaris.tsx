...
/**
 * Animated WebGL noise background. The GL context is created only when the section nears the
 * viewport, the render loop pauses off screen, and reduced-motion users get one static frame.
 */
export default function Velaris({
  ...
  useEffect(() => {
    ...
    let locs: Record<'res' | 'time' | 'grain' | 'colors' | 'bg', WebGLUniformLocation | null> | null = null;
    let raf = 0;
    let visible = false;
    let lastTime = 4000;
    ...
    // The effect is a soft blur, so one device pixel per CSS pixel is plenty.
    function resize() {
      if (!gl) return;
    ...
    const start = () => {
      ...
      if (motionQuery.matches) drawFrame(lastTime);
    ...
    const ro = new ResizeObserver(() => {
      if (!gl) return;
      resize();
      if (motionQuery.matches || !visible) drawFrame(lastTime);
    });
